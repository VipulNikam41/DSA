...
/*int pro_tree(int t,tree *tr,int x1, int y1,int k)
{
	int x2,y2;
	int z2,out;
	float min = 3000;
...
int cut_up(tree *tr,int index,int k)
{
	...
	for(int i =0;i<k;i++){
		if(tr[i].cut ==0 && tr[i].x == tr[index].x && tr[i].y>tr[index].y){
			if(tr[i].y-tr[index].y<tr[index].h)
			{
				dot[p] =i;
				p++;
				//cout<<dot<<endl;
	...
	if(min-tr[index].y<tr[index].h || min ==1000){
		for(int q =0;q<p;q++)
		if(tr[dot[q]].h*tr[dot[q]].d*tr[dot[q]].c<tr[index].h*tr[index].d*tr[index].c && dot>0){
      		value = value+cut_up(tr,dot[q],k);
      	}
	}
	//cout<<value<<endl;
	return value;
...
int cut_down(tree *tr,int index,int k){
	...
	for(int i =0;i<k;i++){
		if(tr[i].cut ==0 && tr[i].x == tr[index].x && tr[i].y<tr[index].y){
			if(abs(tr[i].y-tr[index].y)<tr[index].h)
			{
				dot[p] =i;
				p++;
				//cout<<dot<<endl;
	...
	if(abs(max-tr[index].y)<tr[index].h || max ==0){
		for(int q =0;q<p;q++)
		if(tr[dot[q]].h*tr[dot[q]].d*tr[dot[q]].c<tr[index].h*tr[index].d*tr[index].c && dot>0){
      		value = value+cut_down(tr,dot[q],k);
      	}
	}
	//cout<<value<<endl;
	return value;
...
int cut_right(tree *tr,int index,int k){
	...
	for(int i =0;i<k;i++){
		if(tr[i].cut ==0 && tr[i].y == tr[index].y && tr[i].x>tr[index].x){
			if(tr[i].x-tr[index].x>tr[index].h ){
				dot[p] =i;
				p++;
				//cout<<dot<<endl;
			}
	...
	if(abs(min-tr[index].x)<tr[index].h || min ==1000){
		for(int q =0;q<p;q++)
		if(tr[dot[q]].h*tr[dot[q]].d*tr[dot[q]].c<tr[index].h*tr[index].d*tr[index].c && dot>0){
      		value = value+cut_right(tr,dot[q],k);
      	}
	}
	//cout<<value<<endl;
	return value;
...
int cut_left(tree *tr,int index,int k){
	...
	for(int i =0;i<k;i++){
		if(tr[i].cut ==0 && tr[i].y == tr[index].y && tr[i].x<tr[index].x ){
			if(abs(tr[i].x-tr[index].x)>tr[index].h){
				dot[p] =i;
				p++;
				//cout<<dot<<endl;
			}
	...
	if(abs(max-tr[index].x)<tr[index].h || max ==0){	
		for(int q =0;q<p;q++)
			if(tr[dot[q]].h*tr[dot[q]].d*tr[dot[q]].c<tr[index].h*tr[index].d*tr[index].c){
      			value = value+cut_left(tr,dot[q],k);
